...
function BigBtn({ label, sub, emoji, color, onClick }) {
  const [p, setP] = useState(false);
  return (
    <button onClick={onClick} onMouseDown={() => setP(true)} onMouseUp={() => setP(false)} onTouchStart={() => setP(true)} onTouchEnd={() => setP(false)}
      style={{ width: "100%", padding: "20px 18px", borderRadius: 20, border: "none", background: color, color: G.white, cursor: "pointer", marginBottom: 14, display: "flex", alignItems: "center", gap: 14, textAlign: "left", boxShadow: p ? "0 2px 6px rgba(0,0,0,0.1)" : "0 4px 16px rgba(0,0,0,0.13)", transform: p ? "scale(0.97)" : "scale(1)", transition: "all 0.1s", fontFamily: "Georgia,serif" }}>
      <span style={{ fontSize: 30 }}>{emoji}</span>
      <div>
        <div style={{ fontSize: 19, fontWeight: "bold" }}>{label}</div>
        <div style={{ fontSize: 12, opacity: 0.85, fontStyle: "italic" }}>{sub}</div>
      </div>
    </button>
  );
}
...
function HomeScreen({ onNav, logo }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "100vh", padding: "40px 24px", background: `linear-gradient(160deg, ${G.greenPale} 0%, ${G.cream} 60%)` }}>
      <img
        ...
        alt="PKKAIP Logo"
        style={{ width: 140, height: 140, borderRadius: "50%", objectFit: "cover", marginBottom: 16, boxShadow: "0 4px 20px rgba(45,90,39,0.25)", border: `3px solid ${G.green}`, cursor: "pointer", userSelect: "none", opacity: flash ? 0.7 : 1, transition: "opacity 0.15s" }}
      />
      <div style={{ fontSize: 13, color: G.green, fontWeight: "bold", textAlign: "center", letterSpacing: 0.4, marginBottom: 4 }}>Persatuan Kebun Komuniti Anak Istimewa Puchong</div>
      <div style={{ fontSize: 12, color: G.textLight, fontStyle: "italic", marginBottom: 44 }}>Growing together with love 💚</div>
      <BigBtn emoji="☕" label="Café Order" sub="Take a customer's order" color={G.green} onClick={() => onNav("cafe")} />
      <BigBtn emoji="🌱" label="Garden Plants" sub="Scan or explore plants" color={G.amber} onClick={() => onNav("garden")} />
    </div>
  );
}
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.65)", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
      <div style={{ background: G.white, borderRadius: 24, padding: 28, textAlign: "center", width: "100%", maxWidth: 340 }}>
        ...
        <img src={qrUrl(url)} alt="QR Code" style={{ width: 200, height: 200, border: `4px solid ${G.greenPale}`, borderRadius: 16, marginBottom: 12 }} />
        ...
        {plants.length === 0
          ...
          : plants.map(p => (
            <div key={p.id} style={{ background: G.white, borderRadius: 14, marginBottom: 10, border: `1px solid ${G.greenPale}`, boxShadow: "0 1px 6px rgba(0,0,0,0.05)", overflow: "hidden", display: "flex", alignItems: "stretch" }}>
              ...
              <button onClick={() => setShowQR(p)} style={{ background: G.greenPale, border: "none", borderLeft: `1px solid ${G.greenPale}`, padding: "0 16px", cursor: "pointer", color: G.green, fontSize: 22, flexShrink: 0 }} title="Show QR Code">
                ⬛
              </button>
            </div>
          ))
