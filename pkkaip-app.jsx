...
async function uploadImageToStorage(dataUrl, folder = "general") {
  ...
  const contentType =
    blob.type && String(blob.type).startsWith("image/") ? blob.type : `image/${ext}`;
  ...
  for (const it of items) {
    ...
    if (bucket) {
      bucket.items.push({
        ...
        stock: it.stock === null || it.stock === undefined ? null : Number(it.stock),
      });
    }
  }
...
function menuItemStockUnlimited(stock) {
  return stock === null || stock === undefined;
}

function menuItemSoldOut(item) {
  return !menuItemStockUnlimited(item.stock) && Number(item.stock) <= 0;
}

function findItemInCategories(cats, itemId) {
  for (const c of cats) {
    const found = c.items.find((i) => String(i.id) === String(itemId));
    if (found) return found;
  }
  return null;
}

function patchMenuItemStock(cats, itemId, stock) {
  return cats.map((c) => ({
    ...c,
    items: c.items.map((i) => (i.id === itemId ? { ...i, stock } : i)),
  }));
}

const PKKAIP_STAFF_NAME_KEY = "pkkaip_staff_name";

/** YYYY-MM-DD in local timezone (for admin order date filter). */
function ordersAdminTodayYmd() {
  const d = new Date();
...
/** Local calendar day → inclusive start / exclusive end as ISO for Supabase filters. */
function ordersAdminLocalDayBoundsIso(ymd) {
  if (!/^\d{4}-\d{2}-\d{2}$/.test(ymd)) {
    return ordersAdminLocalDayBoundsIso(ordersAdminTodayYmd());
  }
...
/** e.g. "17 May 2026" from YYYY-MM-DD (local). */
function ordersAdminFormatDayHeading(ymd) {
  if (!/^\d{4}-\d{2}-\d{2}$/.test(ymd)) return ordersAdminTodayYmd();
  const [y, mo, day] = ymd.split("-").map(Number);
...
function ordersAdminAddDays(ymd, delta) {
  const [y, mo, day] = ymd.split("-").map(Number);
  const d = new Date(y, mo - 1, day + delta);
...
function ActionBtn({ label, color, onClick, outline, disabled }) {
  return (
    <button type="button" disabled={disabled} onClick={disabled ? undefined : onClick} style={{ width: "100%", padding: "15px", borderRadius: 14, marginBottom: 10, border: outline ? `1px solid ${G.brownLight}` : "none", background: outline ? G.cream : color, color: outline ? G.brown : G.white, fontSize: 16, fontFamily: "Georgia,serif", fontWeight: "bold", cursor: disabled ? "wait" : "pointer", opacity: disabled ? 0.75 : 1, boxShadow: outline ? "none" : "0 3px 10px rgba(0,0,0,0.12)" }}>
      {label}
    </button>
  );
}
...
function HomeScreen({ onNav, logo }) {
  ...
  return (
      ...
      <img
        ...
        src={logo || BUILTIN_PKKAIP_LOGO_SRC}
        alt="PKKAIP Logo"
...
function persistPendingOrder({ order_number, items, total, paid, change_given, customer_name, staff_name }) {
  void supabase
    .from("orders")
    .insert({
      order_number,
...
function CafeScreen({ onBack, categories, setCategories, logo }) {
  ...
  const [staffDraft, setStaffDraft] = useState(() => {
    try {
      return localStorage.getItem(PKKAIP_STAFF_NAME_KEY) || "";
    } catch {
      return "";
    }
  });

  const cafeHeaderTitle = `☕ Café Order${staffName.trim() ? ` · ${staffName.trim()}` : ""}`;
  ...
  const commitWelcome = () => {
    const c = customerDraft.trim();
    const st = staffDraft.trim();
    setCustomerName(c);
    setStaffName(st);
      ...
      <div style={{ background: G.cream, minHeight: "100vh" }}>
        ...
        <div style={{ padding: "24px 22px 40px", maxWidth: 430, margin: "0 auto" }}>
          ...
          <div style={{ marginBottom: 26 }}>
            ...
            <input
              ...
              onChange={(e) => {
                ...
                setStaffDraft(v);
                try {
                  localStorage.setItem(PKKAIP_STAFF_NAME_KEY, v);
                } catch {
                  /* ignore */
                }
              }}
          ...
          <button
            ...
            style={{ width: "100%", padding: "16px", borderRadius: 14, border: "none", background: G.green, color: G.white, fontSize: 18, fontFamily: "Georgia,serif", fontWeight: "bold", cursor: "pointer", marginBottom: 12, boxShadow: "0 4px 14px rgba(45,90,39,0.25)" }}
          >
            Start Order →
          </button>
          <button
            type="button"
            onClick={commitWelcome}
            style={{ display: "block", width: "100%", background: "none", border: "none", color: G.brownLight, fontSize: 14, fontFamily: "Georgia,serif", cursor: "pointer", textDecoration: "underline", padding: 6 }}
          >
            Skip
          </button>
        </div>
  ...
  if (screen === "payment") return (
    <div>
      <Header title={cafeHeaderTitle} sub={`Payment · Total: RM ${total.toFixed(2)}`} onBack={() => setScreen("summary")} />
  ...
  return (
    <div>
      <Header title={cafeHeaderTitle} sub="Tap items to add to order" onBack={onBack} />
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, padding: 16 }}>
        {(cat?.items || []).map((item) => {
          const live = findItemInCategories(categories, item.id) || item;
          const soldOut = menuItemSoldOut(live);
          const qty = order[item.id]?.qty || 0;
          const hasImg = Boolean(item.image);
          const limitedLabel = !menuItemStockUnlimited(live.stock) && !soldOut
            ? `${live.stock} left`
            : null;
          return (
            <button
              ...
              style={{
                ...
                borderRadius: 16,
                padding: 0,
                border: `2px solid ${soldOut ? "#ccc" : qty > 0 ? G.green : G.greenPale}`,
                cursor: soldOut ? "not-allowed" : "pointer",
                boxShadow: soldOut ? "none" : qty > 0 ? "0 4px 12px rgba(45,90,39,0.25)" : "0 2px 8px rgba(0,0,0,0.06)",
                position: "relative",
                textAlign: "left",
                display: "flex",
            ...
            >
              ...
              <div style={{ padding: hasImg ? "12px 12px 14px" : "14px 12px", flex: 1, background: !soldOut && qty > 0 ? G.green : G.white, display: "flex", flexDirection: "column", justifyContent: "center", borderRadius: hasImg ? "0 0 16px 16px" : 16 }}>
                <div style={{ fontSize: 14, fontWeight: "bold", color: !soldOut && qty > 0 ? G.white : G.text, marginBottom: 4, lineHeight: 1.3 }}>
                  {item.name}
                </div>
                {soldOut && (
                  <div style={{ fontSize: 12, fontWeight: "bold", color: G.red, marginBottom: 4 }}>Out of Stock</div>
                )}
                {!soldOut && limitedLabel && (
                  <div style={{ fontSize: 11, color: qty > 0 ? "rgba(255,255,255,0.85)" : G.textLight, marginBottom: 2, fontStyle: "italic" }}>{limitedLabel}</div>
                )}
                <div style={{ fontSize: 16, fontWeight: "bold", color: !soldOut && qty > 0 ? G.amberLight : G.amber, fontFamily: "monospace" }}>RM {item.price.toFixed(2)}</div>
              </div>
            </button>
          );
        })}
      ...
      {orderItems.length > 0 && (
        <div style={{ background: G.white, margin: "0 16px 24px", borderRadius: 20, padding: 16, boxShadow: "0 2px 12px rgba(0,0,0,0.08)" }}>
          ...
          <ActionBtn label="Clear Order" outline onClick={clearCartWithStockRestore} />
        </div>
      )}
    </div>
  );
...
function ImagePicker({ searchLabel, unsplashQuery, aspectRatio = 1, storageFolder = "general", onSelect, onClose }) {
  ...
  const handleCropUse = async () => {
    ...
    try {
      ...
      let out = dataUrl;
      if (isBase64ImageDataUrl(out)) {
        try {
          out = await uploadImageToStorage(out, storageFolder);
        } catch (err) {
          console.error(err);
          alert("Could not upload image to storage. Try again or check bucket permissions.");
          return;
        }
      }
      onSelect(out);
    ...
    return (
      <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.65)", zIndex: 100, display: "flex", flexDirection: "column", justifyContent: "flex-end" }}>
        <div style={{ background: G.white, borderRadius: "20px 20px 0 0", padding: 20, maxHeight: "90vh", overflow: "hidden", display: "flex", flexDirection: "column" }}>
          ...
          />
          <ActionBtn label={cropBusy ? "Uploading image…" : "Crop & Use"} color={!croppedAreaPixels || cropBusy ? G.brownLight : G.green} onClick={() => { if (!croppedAreaPixels || cropBusy) return; handleCropUse(); }} />
...
function PlantAdmin({ plants, setPlants, onBack }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      try {
        imageUrl = await persistRemoteOrDataImage(form.image?.trim() || "", "plants");
      } catch (e) {
        console.error(e);
      ...
      if (editPlant) {
        const { error } = await supabase.from("plants").update({ ...payload }).eq("id", editPlant.id);
        if (error) { alert(error.message); return; }
        setPlants(ps => ps.map(p => p.id === editPlant.id ? { ...p, ...payload } : p));
      } else {
        const sort_order = plants.length;
        const { data, error } = await supabase.from("plants").insert({ ...payload, sort_order }).select().single();
        if (error) { alert(error.message); return; }
        setPlants(ps => [...ps, plantFromRow(data)]);
      }
      setForm((f) => ({ ...f, image: payload.image }));
      setView("list");
    } finally {
  ...
  if (view === "form") return (
    <div>
      ...
      <div style={{ padding: 20 }}>
        ...
        <ActionBtn label={uploadingSave ? "Uploading image…" : "💾 Save Plant"} color={G.green} onClick={save} disabled={uploadingSave} />
        {editPlant && <>
...
/** iOS-style toggle: on = unlimited stock (null). 44×24 track, white thumb. */
function AdminNoLimitStockToggle({ on, disabled, onToggle }) {
  return (
    <button
      type="button"
      role="switch"
      aria-checked={on}
      aria-label="No limit stock"
      disabled={disabled}
      onClick={onToggle}
      style={{
        width: 44,
...
function MenuAdmin({ categories, setCategories, onBack }) {
  ...
  const [uploadingItemSave, setUploadingItemSave] = useState(false);
  ...
  const applyMenuItemStock = async (itemId, nextStock) => {
    setStockBusy((b) => ({ ...b, [itemId]: true }));
    const { error } = await supabase.from("menu_items").update({ stock: nextStock }).eq("id", itemId);
    setStockBusy((b) => ({ ...b, [itemId]: false }));
    if (error) {
      alert(error.message);
      return;
    }
    setCategories((cats) => patchMenuItemStock(cats, itemId, nextStock));
  };

  const saveItem = async () => {
    if (!itemForm.name.trim() || !itemForm.price) return;
    setUploadingItemSave(true);
    const price = parseFloat(itemForm.price);
    try {
      let image = itemForm.image?.trim() || "";
      try {
        image = await persistRemoteOrDataImage(itemForm.image?.trim() || "", "menu");
      } catch (e) {
        console.error(e);
        alert(e?.message ?? "Could not upload image.");
        return;
      ...
      if (editItem) {
        ...
        const { data, error } = await supabase.from("menu_items").insert({ category_id: editCat.id, name: itemForm.name, price, sort_order, image, stock: null }).select().single();
        if (error) { alert(error.message); return; }
        setCategories(cats => cats.map(c => {
          if (c.id !== editCat.id) return c;
          return { ...c, items: [...c.items, { id: data.id, name: data.name, price: Number(data.price), image: data.image ?? "", sort_order: Number(data.sort_order) || sort_order, stock: data.stock === null || data.stock === undefined ? null : Number(data.stock) }] };
        }));
      }
      setView("editCat");
    } finally {
      setUploadingItemSave(false);
    }
  };
  ...
  if (view === "itemForm") return (
    <div>
      ...
      <div style={{ padding: 20 }}>
        ...
        <ActionBtn label={uploadingItemSave ? "Uploading image…" : "Save"} color={G.green} onClick={saveItem} disabled={uploadingItemSave} />
                  ...
                  {({ setNodeRef, attributes, listeners, dndStyle }) => (
                    ...
                    >
                      <button
                        ...
                        style={{
                          ...adminDragHandleStyle,
                          width: 36,
                          minHeight: 0,
                          fontSize: 18,
                          borderRight: `1px solid ${G.greenPale}`,
                        }}
                      >
                        ≡
                      </button>
                      ...
                      >
                        ...
                        >
                          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                            <AdminNoLimitStockToggle
                              on={menuItemStockUnlimited(item.stock)}
                              disabled={!!stockBusy[item.id]}
                              onToggle={() => {
                                if (stockBusy[item.id]) return;
                                if (menuItemStockUnlimited(item.stock)) void applyMenuItemStock(item.id, 0);
                                else void applyMenuItemStock(item.id, null);
                              }}
                            />
                            <span
                              style={{
                          ...
                          {menuItemStockUnlimited(item.stock) ? (
                            <div style={{ fontSize: 13, color: G.green, fontWeight: 600, fontFamily: "Georgia,serif", paddingLeft: 2 }}>
                              No limit ✓
                            </div>
                          ) : (
                            <div
                            ...
                            >
                              <span style={{ fontSize: 12, color: G.textLight, fontFamily: "Georgia,serif", flexShrink: 0 }}>
                                Stock:
                              </span>
                              <button
                                type="button"
                                disabled={!!stockBusy[item.id] || Number(item.stock) <= 0}
                                onClick={() => {
                                  const s = Number(item.stock);
                                  if (s <= 0) return;
                                  void applyMenuItemStock(item.id, s - 1);
                                }}
                                style={{
                                  width: 32,
                                  height: 32,
                                  ...
                                  fontSize: 20,
                                  fontWeight: "bold",
                                  cursor: stockBusy[item.id] || Number(item.stock) <= 0 ? "not-allowed" : "pointer",
                                  flexShrink: 0,
                                  lineHeight: 1,
                                  padding: 0,
                                  display: "flex",
                                  alignItems: "center",
                                  justifyContent: "center",
                                  opacity: Number(item.stock) <= 0 ? 0.5 : 1,
                                }}
                              >
                                −
                              </button>
                              <input
                                key={`${item.id}-stk-${item.stock}`}
                                type="number"
                                inputMode="numeric"
                                defaultValue={String(Number(item.stock))}
                                disabled={!!stockBusy[item.id]}
                                onBlur={(e) => {
                                  if (menuItemStockUnlimited(item.stock)) return;
                                  const v = e.target.value.trim();
                                  const n = v === "" ? 0 : Math.max(0, Math.floor(Number(v)));
                                  if (Number.isNaN(n)) return;
                                  if (n !== Number(item.stock)) void applyMenuItemStock(item.id, n);
                                }}
                                onKeyDown={(e) => {
                              ...
                              <button
                                type="button"
                                disabled={!!stockBusy[item.id]}
                                onClick={() => void applyMenuItemStock(item.id, Number(item.stock) + 1)}
                                style={{
                                  width: 32,
                                  height: 32,
                                  borderRadius: "50%",
                                  ...
                                  fontSize: 20,
                                  fontWeight: "bold",
                                  cursor: stockBusy[item.id] ? "wait" : "pointer",
                                  flexShrink: 0,
                                  lineHeight: 1,
                                  padding: 0,
                                  display: "flex",
...
function LogoAdmin({ logo, setLogo, onBack }) {
  ...
  const save = async () => {
    try {
      ...
      let valueToStore = preview;
      if (preview != null && String(preview).trim() !== "" && isBase64ImageDataUrl(preview)) {
        try {
          valueToStore = await uploadImageToStorage(preview, "logo");
        } catch (e) {
          console.error(e);
          alert(e?.message ?? "Could not upload logo image.");
          return;
        }
        setPreview(valueToStore);
      }
      if (valueToStore) {
        const { error } = await supabase.from("app_settings").upsert({ key: "logo", value: valueToStore }, { onConflict: "key" });
        if (error) { alert(error.message); return; }
        setLogo(valueToStore);
      }
      onBack();
    } finally {
  ...
  return (
    <div>
      ...
      <div style={{ padding: 20 }}>
        ...
        <ActionBtn label={uploadingLogo ? "Uploading image…" : "✓ Save Logo"} color={G.green} onClick={save} disabled={uploadingLogo} />
...
function buildOrdersDaySummary(rows) {
  const map = new Map();
  for (const o of rows) {
  ...
  const items = [...map.entries()].map(([name, v]) => ({ name, qty: v.qty, revenue: v.revenue }));
  items.sort((a, b) => b.qty - a.qty || b.revenue - a.revenue);
  const totalOrders = rows.length;
  const totalRevenue = rows.reduce((s, o) => s + Number(o.total ?? 0), 0);
  const mostPopular = items.length ? items[0] : null;
  const bestRevenue = items.reduce((best, row) => (!best || row.revenue > best.revenue ? row : best), null);
...
function OrdersDaySummaryModal({ onClose, rows, dateLabel, dateYmd, selectedDateIsToday }) {
  ...
  const cellBase = { fontSize: 16, padding: "10px 8px", fontFamily: "Georgia,serif", color: G.text };
  const money = (n) => `RM ${Number(n).toFixed(2)}`;

  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 300, background: G.white, overflowY: "auto", WebkitOverflowScrolling: "touch" }}>
      <div
        style={{
          position: "sticky",
          top: 0,
          ...
          padding: "14px 16px",
          background: G.white,
          borderBottom: `2px solid ${G.greenPale}`,
      ...
      <div style={{ padding: "24px 16px 48px", maxWidth: 560, margin: "0 auto" }}>
        ...
        >
          ...
          <div style={{ display: "flex", gap: 14, marginBottom: 28, flexWrap: "wrap" }}>
            ...
            >
              <div style={{ fontSize: 16, fontWeight: "bold", color: G.green, fontFamily: "Georgia,serif", marginBottom: 8 }}>Total Orders</div>
              <div style={{ fontSize: 36, fontWeight: "bold", color: G.green, fontFamily: "Georgia,serif", lineHeight: 1.1 }}>{summary.totalOrders}</div>
            </div>
            <div
              style={{
                flex: "1 1 200px",
            ...
            >
              <div style={{ fontSize: 16, fontWeight: "bold", color: G.amber, fontFamily: "Georgia,serif", marginBottom: 8 }}>Total Revenue (RM)</div>
              <div style={{ fontSize: 36, fontWeight: "bold", color: G.amber, fontFamily: "monospace", lineHeight: 1.1 }}>{summary.totalRevenue.toFixed(2)}</div>
            </div>
          </div>

          <div style={{ fontSize: 20, fontWeight: "bold", color: G.green, fontFamily: "Georgia,serif", marginBottom: 14 }}>🍽️ Items Sold</div>
          <div style={{ overflowX: "auto", marginBottom: 28 }}>
            <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 280 }}>
              <thead>
                <tr style={{ borderBottom: `2px solid ${G.greenPale}` }}>
                  <th style={{ ...cellBase, textAlign: "left", fontWeight: "bold", color: G.green }}>Item</th>
                  <th style={{ ...cellBase, textAlign: "right", fontWeight: "bold", color: G.green }}>Qty</th>
                  <th style={{ ...cellBase, textAlign: "right", fontWeight: "bold", color: G.green }}>Revenue</th>
                </tr>
              </thead>
              <tbody>
                {summary.items.length === 0 ? (
                  <tr>
                    <td colSpan={3} style={{ ...cellBase, textAlign: "center", color: G.textLight, fontStyle: "italic", padding: "16px 8px" }}>
                      No line items for this day
                    </td>
                  </tr>
                ) : (
                  summary.items.map((row) => (
                    <tr key={row.name} style={{ borderBottom: `1px solid ${G.greenPale}` }}>
                      <td style={{ ...cellBase, textAlign: "left" }}>{row.name}</td>
                      <td style={{ ...cellBase, textAlign: "right", fontWeight: "bold" }}>{row.qty}</td>
                      <td style={{ ...cellBase, textAlign: "right", fontFamily: "monospace", color: G.amber, fontWeight: "bold" }}>{money(row.revenue)}</td>
                    </tr>
                  ))
                )}
              </tbody>
          ...
          <div style={{ fontSize: 20, fontWeight: "bold", color: G.green, fontFamily: "Georgia,serif", marginBottom: 12 }}>💰 Highlights</div>
          <div style={{ fontSize: 17, color: G.text, fontFamily: "Georgia,serif", lineHeight: 1.55, marginBottom: 8 }}>
            <span style={{ fontWeight: "bold", color: G.green }}>Most popular:</span>{" "}
            {summary.mostPopular ? `${summary.mostPopular.name} (${summary.mostPopular.qty} sold)` : "—"}
          </div>
          <div style={{ fontSize: 17, color: G.text, fontFamily: "Georgia,serif", lineHeight: 1.55, marginBottom: 24 }}>
            <span style={{ fontWeight: "bold", color: G.amber }}>Best revenue:</span>{" "}
            {summary.bestRevenue ? `${summary.bestRevenue.name} (${money(summary.bestRevenue.revenue)})` : "—"}
          </div>

          <div style={{ fontSize: 13, color: G.textLight, textAlign: "center", marginBottom: 16, fontFamily: "Georgia,serif" }}>Generated by PKKAIP App</div>
          <div style={{ fontSize: 14, color: G.textLight, fontStyle: "italic", textAlign: "center", fontFamily: "Georgia,serif", lineHeight: 1.5 }}>
            Take a screenshot to save or share
          </div>
        </div>
      </div>
    </div>
...
function OrdersAdmin({ onBack }) {
  ...
  const load = useCallback(async () => {
    ...
    const { data, error } = await supabase
      .from("orders")
      .select("id, order_number, created_at, status, total, items, staff_name, customer_name")
  ...
  const statusStyle = (status) => {
    ...
    if (s === "cancelled") return { background: "#fee2e2", color: "#991b1b", border: "1px solid #fecaca" };
  ...
  const dateLabel = ordersAdminFormatDayHeading(dateYmd);
      ...
      <div style={{ padding: 16 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10, marginBottom: 12 }}>
          <button
            ...
            style={navBtnStyle}
            onClick={() => setDateYmd((d) => ordersAdminAddDays(d, -1))}
          >
            ←
          </button>
          <input
        ...
        >
          {orderCount} order{orderCount !== 1 ? "s" : ""} · RM {totalRevenue.toFixed(2)}
        </div>
        ...
        {loading ? (
          ...
          >
            ...
            <div style={{ fontSize: 17, fontWeight: "bold", color: G.text, marginBottom: 8 }}>No orders on this day</div>
            <div style={{ fontSize: 14, fontStyle: "italic", lineHeight: 1.5, maxWidth: 320, margin: "0 auto" }}>
              Nothing was recorded for {dateLabel}. Try another date with the arrows or calendar, or check back after the café makes sales.
            </div>
          </div>
        ) : (
          rows.map((o) => {
            const staff = typeof o.staff_name === "string" ? o.staff_name.trim() : "";
            const customer = typeof o.customer_name === "string" ? o.customer_name.trim() : "";
            return (
              <div
                key={o.id}
                style={{
              ...
              >
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10, gap: 8 }}>
                  <div style={{ minWidth: 0 }}>
                    <div style={{ fontSize: 17, fontWeight: "bold", color: G.green, fontFamily: "Georgia,serif" }}>
                      Order #{o.order_number}
                    </div>
                    <div style={{ fontSize: 13, color: G.textLight, marginTop: 2 }}>{formatOrderTime(o.created_at)}</div>
                  </div>
                  <span
                    style={{
                      fontSize: 11,
                ...
                {(staff || customer) ? (
                  <div style={{ fontSize: 12, marginBottom: 10, lineHeight: 1.45 }}>
                    {customer ? (
                      <div style={{ fontSize: 14, fontWeight: "bold", color: G.amber, fontFamily: "Georgia,serif", marginBottom: 4 }}>
                        For: {customer}
                      </div>
                    ) : null}
                    {staff ? (
                      <div style={{ color: G.textLight }}>
                        <span style={{ fontWeight: "bold", color: G.text }}>Staff:</span> {staff}
                      </div>
                    ) : null}
                  </div>
                ) : null}
                ...
                <div style={{ fontSize: 16, fontWeight: "bold", fontFamily: "monospace", color: G.amber }}>
        ...
        <ActionBtn label="Refresh" outline onClick={() => void load()} />
      </div>
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <div style={{ padding: "28px 20px" }}>
        ...
        <BigBtn emoji="📋" label="Orders History" sub="Browse café orders by date" color={G.greenLight} onClick={() => setView("orders")} />
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  return (
    <div style={{ fontFamily: "Georgia,serif", background: G.cream, minHeight: "100vh", maxWidth: 430, margin: "0 auto" }}>
      ...
      {screen === "cafe"   && <CafeScreen onBack={() => setScreen("home")} categories={categories} setCategories={setCategories} logo={logo} />}
